...
def main(tv_file, vocab_file):
    ...
    # Create a normal feature matrix for all target function words
    ...
    train, val = split_dataset(line_features, binary_features, labels, 0.9, line_features)

    # Run GaussianNB on features
    model_1 = GaussianNB()
    model_1.fit(train[0], train[3])
    print(f"Gaussian Classifier score: {model_1.score(val[0], val[3])}")  # print score
...
if __name__ == '__main__':
    ...
    main(args.tv_path, args.function_words_path)